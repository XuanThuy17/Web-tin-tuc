...
import { fetchGetNewsByCate } from "../../../services/public";
...
const AllNewsCate = () => {
  ...
  const displayListNew = async (id) => {
    let res = await fetchGetNewsByCate(id, page, limit);
    if (res && res.EC === 0) {
        ...
        <div className="container">
          ...
          {getNews.slice(0,1).map((news, index) => (
            <div className="col-12 col-sm-9" key={index}>
              <h4>{news.Category_detail.Category.name}</h4>
            </div>
          ))}
